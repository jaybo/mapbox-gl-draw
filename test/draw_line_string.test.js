...
// jaybo: deliberate finish taps in these tests must clear the duplicate-tap window
const tapDuplicateGapMS = 200;
...
test('draw_line_string touch interaction', async (t) => {
  ...
  await t.test('add more points then tap on the last vertex to finish', async () => {
    touchTap(map, makeTouchEvent(400, 500));
    touchTap(map, makeTouchEvent(300, 500));
    touchTap(map, makeTouchEvent(200, 500));
    // jaybo: clear the duplicate-tap window so the finish tap is deliberate
    await sleep(tapDuplicateGapMS);
    touchTap(map, makeTouchEvent(200, 500));
    const line = Draw.getAll().features[0];
    assert.deepEqual(line.geometry.coordinates,
